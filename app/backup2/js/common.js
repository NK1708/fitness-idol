	...
	$(document).ready(function() {
      ...
      $('.generator__btn').on('click tap touch', function() { 
        ...
        if($this.hasClass("MuteOn")) { 
          ...
          $('.generator__btn').text('Найти другую причину');
...
function generationCanvas() {
    ...
    ctx.clearRect(0, 0, 673, 378);

    pic = new Image(673, 378);              
    pic.src = ('./img/default.png');
    text = $('#generator__box').text().replace(/(<([^>]+)>)/ig,"");
    pic.onload = function(){
      ctx.drawImage(pic, 0, 0, 673, 378);
      ...
      ctx.fillText( text.toUpperCase(), 336, 230);
      ...
      $.ajax({
        ...
        $('.vk').click(function(){
          setUrl('vk', 'https://vk.com/share.php?url=http://idol.drivefitness.ru/?resultKey=' + filename);
        });

        $('.fb').click(function(){
          setUrl('fb', 'http://www.facebook.com/sharer/sharer.php?http://idol.drivefitness.ru/?resultKey=' + filename);
        });
      });
    }
}
